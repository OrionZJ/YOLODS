...
from lib.core.function import multi_task_trainer, multi_task_validater
...
def main():
    ...
    try:
        avg_cost = np.load('./runs/avg_cost.npy')
        lambda_weight = np.load('./runs/lambda_weight.npy')
        if avg_cost.shape[0] < cfg.TRAIN.END_EPOCH:
            new_row = np.zeros([cfg.TRAIN.END_EPOCH - avg_cost.shape[0], 3], dtype=np.float32)
            avg_cost = np.vstack(avg_cost, new_row)
        if lambda_weight.shape[1] < cfg.TRAIN.END_EPOCH:
            new_col = np.ones([3, cfg.TRAIN.END_EPOCH - lambda_weight.shape[1]])
            lambda_weight = np.hstack(lambda_weight, new_col)
    except FileNotFoundError:
        avg_cost = np.zeros([cfg.TRAIN.END_EPOCH, 3], dtype=np.float32)
        lambda_weight = np.ones([3, cfg.TRAIN.END_EPOCH])
    ...
    for epoch in range(begin_epoch + 1, cfg.TRAIN.END_EPOCH + 1):
        ...
        losses_rec = multi_task_trainer(cfg, train_loader, model, criterion, optimizer, scaler,
                                        epoch, num_batch, num_warmup, writer_dict, logger, device, lambda_weight, index,
                                        rank)

        avg_cost[index, 0] = losses_rec[0] + losses_rec[1] + losses_rec[2]
        avg_cost[index, 1] = losses_rec[3]
        avg_cost[index, 2] = losses_rec[4] + losses_rec[5]
        np.save('./runs/avg_cost.npy', avg_cost)
        ...
        if (epoch % cfg.TRAIN.VAL_FREQ == 0 or epoch == cfg.TRAIN.END_EPOCH) and rank in [-1, 0]:
            print('validate:')
            da_segment_results, ll_segment_results, detect_results, total_loss, maps, times = multi_task_validater(
                epoch, cfg, valid_loader, valid_dataset, model, criterion,
                final_output_dir, tb_log_dir, writer_dict,
                logger, device, lambda_weight, index, rank
            )
