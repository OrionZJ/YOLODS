...
class MultiHeadLoss_dwa(nn.Module):
    """
    collect all the loss we need
    """
    ...
    def forward(self, head_fields, head_targets, shapes, model, lambdas=None):
        """
        Inputs:
        - head_fields: (list) output from each task head
        - head_targets: (list) ground-truth for each task head
        ...
        if lambdas is not None:
            if type(lambdas).__name__ != 'list':
                lambdas = lambdas.tolist()
            if len(lambdas) == 3:
                lambdas_copy = [1.0 for _ in range(len(lambdas) + 3)]
                lambdas_copy[0] = lambdas[0]
                lambdas_copy[1] = lambdas[0]
                lambdas_copy[2] = lambdas[0]
                lambdas_copy[3] = lambdas[1]
                lambdas_copy[4] = lambdas[2]
                lambdas_copy[5] = lambdas[2]
                self.lambdas = lambdas_copy
            elif len(lambdas) == 6:
                pass
...
def get_loss(cfg, device):
    ...
    loss = MultiHeadLoss_dwa(loss_list, cfg=cfg, lambdas=cfg.LOSS.MULTI_HEAD_LAMBDA)
    return loss
